    ...
    private inner class SettingButton(private val setting: UserSetting) : Button() {
        ...
        override fun getDescription(player: Player): List<String> {
            ...
            for (option in setting.getOptions<UserSettingOption>()) {
                if (user.getSettingOption<UserSettingOption>(setting) == option) {
        ...
        override fun clicked(player: Player, slot: Int, clickType: ClickType, view: InventoryView) {
            if (clickType.isLeftClick) {
                user.updateSettingOption<UserSettingOption>(setting, setting.getPreviousValue(user.getSettingOption(setting)))
            } else if (clickType.isRightClick) {
                user.updateSettingOption<UserSettingOption>(setting, setting.getNextValue(user.getSettingOption(setting)))
            }
        }
    }
