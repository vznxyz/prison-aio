...
package net.evilblock.prisonaio.module.auction.menu.creation
...
class SelectListingTypeMenu(private val select: (ListingType) -> Unit) : Menu() {
    ...
    private inner class PurchaseTypeButton : TexturedHeadButton(PURCHASE_TEXTURE) {
        ...
        override fun clicked(player: Player, slot: Int, clickType: ClickType, view: InventoryView) {
            if (clickType.isLeftClick) {
                select.invoke(ListingType.PURCHASE)
            }
        }
    }
    ...
    private inner class AuctionTypeButton : TexturedHeadButton(AUCTION_TEXTURE) {
        ...
        override fun clicked(player: Player, slot: Int, clickType: ClickType, view: InventoryView) {
            if (clickType.isLeftClick) {
                select.invoke(ListingType.AUCTION)
            }
        }
    }
