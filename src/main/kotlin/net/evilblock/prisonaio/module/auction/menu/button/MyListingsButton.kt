...
class MyListingsButton : Button() {
    ...
    override fun getDescription(player: Player): List<String> {
        return arrayListOf<String>().also {
            ...
            val activeListings = AuctionHouseHandler.getPlayerListings(player.uniqueId).filter { listing -> !listing.isCompleted() }.size
            it.add("${ChatColor.GRAY}Active Listings: ${ChatColor.GREEN}${ChatColor.BOLD}${NumberUtils.format(activeListings)}")

            val completedListings = AuctionHouseHandler.getPlayerListings(player.uniqueId).filter { listing -> listing.isCompleted() }.size
