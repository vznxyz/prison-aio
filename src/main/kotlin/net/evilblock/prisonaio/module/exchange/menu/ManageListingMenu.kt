...
class ManageListingMenu(private val previousMenu: Menu? = null, private val listing: GrandExchangeListing) : Menu() {
    ...
    private inner class UpdateBINPriceButton : TexturedHeadButton(texture = Constants.GOLD_DOLLAR_TEXTURE) {
        ...
        override fun getDescription(player: Player): List<String> {
            return arrayListOf<String>().also {
                ...
                if (listing.listingType == GrandExchangeListingType.AUCTION && listing.getLatestBid() != null) {
    ...
    private inner class FeatureListingButton : Button() {
        ...
        override fun clicked(player: Player, slot: Int, clickType: ClickType, view: InventoryView) {
            if (clickType.isLeftClick) {
                ...
                ConfirmMenu(
                    title = "Feature for ${Formats.formatTokens(GrandExchangeModule.getFeatureListingPrice())}${ChatColor.RESET}?",
                    extraInfo = TextSplitter.split(text = "Pay to feature your listing for a fee of ${Formats.formatTokens(GrandExchangeModule.getFeatureListingPrice())}${ChatColor.GRAY}.")
                ) { confirmed ->
                    if (confirmed) {
                        listing.setFeatured()
        ...
        private fun canFeatureListing(player: Player, listing: GrandExchangeListing): Boolean {
            ...
            if (!Currency.Tokens.has(player.uniqueId, GrandExchangeModule.getFeatureListingPrice())) {
                player.sendMessage("${ChatColor.RED}You need ${Formats.formatTokens(GrandExchangeModule.getFeatureListingPrice())} ${ChatColor.RED}to feature your listing!")
                return false
            }
    ...
    private inner class DeleteListingButton : Button() {
        ...
        override fun getDescription(player: Player): List<String> {
            return arrayListOf<String>().also {
                it.add("")
                it.addAll(TextSplitter.split(text = "Delete your listing for a deletion fee of ${Formats.formatTokens(GrandExchangeHandler.DELETION_FEE)}${ChatColor.GRAY}. Your item will be safely returned and any bid(s) will be refunded."))
                it.add("")
            ...
            if (clickType.isLeftClick) {
                ConfirmMenu { confirmed ->
                    if (confirmed) {
                        ...
                        if (!Currency.Tokens.has(player.uniqueId, GrandExchangeHandler.DELETION_FEE)) {
                            player.sendMessage("${ChatColor.RED}You don't have enough tokens to pay the deletion fee of ${Formats.formatTokens(GrandExchangeHandler.DELETION_FEE)}${ChatColor.RED}!")
                            this@ManageListingMenu.openMenu(player)
                            return@ConfirmMenu
                        }

                        Currency.Tokens.take(player.uniqueId, GrandExchangeHandler.DELETION_FEE)
                        ...
                        Tasks.async {
                            ...
                            for ((uuid, bid) in returnedBids) {
                                ...
                                user.grandExchangeData.addNotification(GrandExchangeUserNotification(message = "${ChatColor.GRAY}Your bid of ${listing.getCurrencyType().format(bid.amount)} ${ChatColor.GRAY}on ${ChatColor.RED}${listing.getCreatorUsername()}${ChatColor.GRAY}'s auction has been returned!"))
