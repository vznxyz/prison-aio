...
package net.evilblock.prisonaio.module.exchange.listing
...
class GrandExchangeListing(
    val listingType: GrandExchangeListingType,
    ...
    private var bidHistory: LinkedList<GrandExchangeListingBid> = LinkedList()
    ...
    fun isBINOnly(): Boolean {
        return listingType == GrandExchangeListingType.PURCHASE
    }

    fun isAuction(): Boolean {
        return listingType == GrandExchangeListingType.AUCTION
    }
    ...
    fun getBidHistory(): List<GrandExchangeListingBid> {
        return bidHistory
    }

    fun addBid(bid: GrandExchangeListingBid) {
        bidHistory.add(bid)
        requiresSave = true
    }

    fun getLatestBid(): GrandExchangeListingBid? {
        return bidHistory.maxBy { it.createdAt }
    }

    fun getLastBid(playerUuid: UUID): GrandExchangeListingBid? {
        return bidHistory.filter { it.createdBy == playerUuid }.maxBy { it.createdAt }
    }

    class ListingParameterType : ParameterType<GrandExchangeListing> {
        override fun transform(sender: CommandSender, source: String): GrandExchangeListing? {
            return try {
                GrandExchangeHandler.getListingById(UUID.fromString(source))
            } catch (e: Exception) {
                null
            }
        }
