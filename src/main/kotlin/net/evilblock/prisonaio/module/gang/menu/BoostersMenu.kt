...
class BoostersMenu(private val gang: Gang) : Menu() {
    ...
    override fun getTitle(player: Player): String {
        return "${ChatColor.GOLD}${ChatColor.BOLD}Gang Boosters"
    }

    override fun getButtons(player: Player): Map<Int, Button> {
        val buttons = hashMapOf<Int, Button>()

        buttons[0] = BackButton { GangMenu(gang).openMenu(player) }
            ...
            if (clickType.isLeftClick) {
                ...
                if (gang.hasTrophies(boosterType.price)) {
                    ConfirmMenu { confirmed ->
                        ...
                        this@BoostersMenu.openMenu(player)
                    }.openMenu(player)
                } else {
